...
class ContactForm(forms.Form):
	name = forms.CharField(label="Navn", max_length="30", min_length="3", required=True, widget=forms.TextInput(attrs={'placeholder': 'Dit navn'}))
	email = forms.EmailField(label="Email", required=True, widget=forms.TextInput(attrs={'placeholder': 'Din email'}))
	subject = forms.CharField(label="Subject", required=True, widget=forms.TextInput(attrs={'placeholder': 'Emne'}))
