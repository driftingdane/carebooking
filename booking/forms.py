...
class BookingSettingsForm(ChangeInputsStyle, forms.ModelForm):
    start_time = forms.TimeField(widget=forms.TimeInput(format='%H:%M'), help_text="Valid format: 00:00")
    end_time = forms.TimeField(widget=forms.TimeInput(format='%H:%M'), help_text="Valid format: 00:00")
...
class BookingDisableDaysForm(ChangeInputsStyle, forms.Form):
    disable_days = forms.CharField(label=False, required=False, widget=forms.TextInput(attrs={'id': 'disable_days', 'readonly': 'readonly', 'placeholder': 'Click to choose dates'}))

    class Meta:
        model = BookingSettings
        fields = ("disable_dates", "available_booking_months",)
        exclude = [
